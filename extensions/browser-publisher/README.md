...
> 当前版本是扩展本地 DOM 适配基础：选择器、协议校验和安全边界已有自动化测试，但桌面发布队列到扩展的认证传输尚未接线，也尚未使用 CSDN、今日头条、微信公众号的真实账号完成端到端验证。平台页面改版后，扩展可能返回 `NEEDS_USER`，需要人工处理或更新适配器。
...
5. 在弹窗输入“模拟草稿标题/正文”并点击填充。弹窗会生成 `FILL_DRAFT` 任务；任务包含
